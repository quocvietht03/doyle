<?php
class doyle_Icon_Text_Widget extends WP_Widget {
    function __construct() {
        parent::__construct(
                'icon_text_widget', // Base ID
                __('Icon Text', 'doyle'), // Name
                array('description' => __('Icon Text Widget', 'doyle'),) // Args
        );
    }
    ...
    function form($instance) {
		...
		?>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('column')); ?>"><?php _e('Column:', 'doyle'); ?></label>
        ...
        for ($i = 1; $i <= 3; $i++) {
            ?>
            <p>
                <label for="<?php echo esc_url($this->get_field_id('icon_fa_cc_' . $i)); ?>"><?php _e('Icon:', 'doyle');
            echo $i; ?></label>
            ...
            <p>
                <label for="<?php echo esc_url($this->get_field_id('text_fa_cc_' . $i)); ?>"><?php _e('Text:', 'doyle');
            echo $i; ?></label>
 ...
 */
function doyle_register_icon_text_widget() {
    register_widget('doyle_Icon_Text_Widget');
}
add_action('widgets_init', 'doyle_register_icon_text_widget');
?>
