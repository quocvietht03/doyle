<?php
class doyle_Woo_Filter_Attribute extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'doyle' );
		$this->widget_id          = 'woocommerce_layered_nav';
		$this->widget_name        = __( 'WooCommerce Filter Attribute', 'doyle' );
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'doyle' ),
				'label' => __( 'Title', 'doyle' )
			),
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => __( 'Attribute', 'doyle' ),
				'options' => $attribute_array
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => __( 'Display type', 'doyle' ),
				'options' => array(
					'list'     => __( 'List', 'doyle' ),
					'dropdown' => __( 'Dropdown', 'doyle' ),
					'button' => __( 'Button', 'doyle' )
				)
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'doyle' ),
				'options' => array(
					'and' => __( 'AND', 'doyle' ),
					'or'  => __( 'OR', 'doyle' )
				)
		...
		if ( 0 < count( $terms ) ) {
			...
			if ( 'dropdown' == $display_type ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'doyle' ), wc_attribute_label( $taxonomy ) ) . '</option>';
...
/**
 * Class doyle_Woo_Filter_Attribute
 */
function register_woo_filter_attribute_widget() {
    register_widget('doyle_Woo_Filter_Attribute');
}
