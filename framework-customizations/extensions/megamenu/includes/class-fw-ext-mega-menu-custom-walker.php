...
class FW_Ext_Mega_Menu_Custom_Walker extends Walker_Nav_Menu
{
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ($item_type = fw_ext_mega_menu_get_db_item_option($item->ID, 'type')) {
			...
			if (isset($extra_options['menu_item_padding'])&&$extra_options['menu_item_padding']) {
				$style_arr[] = 'padding: ' . $extra_options['menu_item_padding'];
			}
			
			if (isset($extra_options['sub_menu_align'])&&$extra_options['sub_menu_align']) {
				$classes[] = $extra_options['sub_menu_align'];
			}
			
			if ($depth > 0 && fw_ext_mega_menu_get_meta($item, 'title-off')) {
				$classes[] = 'hidden-title';
			}
			if(isset($extra_options['menu_mega_type']['type'])&&$extra_options['menu_mega_type']['type']){
				$classes[] = $extra_options['menu_mega_type']['type'];
			}
			
			if (isset($extra_options['menu_mega_column_width'])&&$extra_options['menu_mega_column_width']) {
				$style_arr[] = 'width: ' . $extra_options['menu_mega_column_width'];
			}
			if (isset($extra_options['menu_mega_column_padding'])&&$extra_options['menu_mega_column_padding']) {
				$style_arr[] = 'padding: ' . $extra_options['menu_mega_column_padding'];
			}
		}
		
		if(!empty($style_arr)){
			$extra_style = ' style="' . esc_attr( implode('; ', $style_arr) ) . '"';
		}
		...
		$output .= $indent . '<li' . $id . $class_names . $extra_style .'>';
...
# END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	function display_element( $element, &$children_elements, $max_depth, $depth, $args, &$output ) {
		...
		// extra options
		$extra_options = $style_arr = $class_arr = array();
		...
		if ($item_type = fw_ext_mega_menu_get_db_item_option($id, 'type')) {
			...
			if(isset($extra_options['menu_mega_type']['type'])&&$extra_options['menu_mega_type']['type'] == 'fullwidth'){
				...
				if(isset($extra_options['menu_mega_type']['customize']['menu_mega_container_position'])&&$extra_options['menu_mega_type']['customize']['menu_mega_container_position']){
					$class_arr[] = $extra_options['menu_mega_type']['customize']['menu_mega_container_position'];
				}
			}
			
			if(isset($extra_options['menu_mega_container_bg']['type'])&&$extra_options['menu_mega_container_bg']['type'] == 'color'){
				...
				if(isset($extra_options['menu_mega_container_bg']['image']['bg_image_size'])&&$extra_options['menu_mega_container_bg']['image']['bg_image_size']){
					$style_arr[] = 'background-size: '. $extra_options['menu_mega_container_bg']['image']['bg_image_size'];
				}
				if(isset($extra_options['menu_mega_container_bg']['image']['bg_image_position'])&&$extra_options['menu_mega_container_bg']['image']['bg_image_position']){
					$style_arr[] = 'background-position: '. $extra_options['menu_mega_container_bg']['image']['bg_image_position'];
				}
			}
			
			$extra_style = implode('; ', $style_arr);
			$extra_class = implode(' ', $class_arr);
		}
...
# END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	function start_lvl( &$output, $depth = 0, $args = array(), $class = 'sub-menu', $extra_options = array() ) {
		...
		if(isset($extra_options['sub_menu_padding'])&&$extra_options['sub_menu_padding']) {
			$style_arr[] = 'padding: '.$extra_options['sub_menu_padding'];
		}
		
		
		if(isset($extra_options['menu_mega_row_padding'])&&$extra_options['menu_mega_row_padding']) {
			$style_arr[] = 'padding: '.$extra_options['menu_mega_row_padding'];
		}
